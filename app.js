...
function disconnect(message = "Enter your API key") {
  ...
  clearInterval(state.websocketPingTimer);
...
async function bootstrap() {
  ...
  connectWebSocket();
  scheduleServerSync(350);
  refreshModels().catch(() => {});
}

async function ensureCurrentChat() {
  const storedChatExists = Boolean(
    state.currentChatId && state.chats.some(chat => chat.id === state.currentChatId)
  );

  if (storedChatExists) {
    await loadChat(state.currentChatId);
  } else {
    // A fresh browser/device gets its own empty chat instead of silently joining
    // another device's currently newest conversation. Existing chats remain in
    // the sidebar and can still be opened deliberately.
...
function scheduleServerSync(delay = null) {
  clearTimeout(state.syncTimer);
  if (!state.connected) return;
  const nextDelay = delay ?? (
    state.activeRun ? (websocketIsReady() ? 2200 : 850) :
    websocketIsReady() ? 8000 : 1800
  );
  state.syncTimer = setTimeout(() => syncFromServer().catch(() => {}), nextDelay);
}

async function syncFromServer() {
  if (!state.connected || state.syncInFlight) {
    scheduleServerSync();
    return;
  }
  ...
  try {
    ...
    if (currentStillExists) {
      state.currentChat = await api(`/api/chats/${encodeURIComponent(state.currentChatId)}`);
    } else if (state.currentChatId) {
      state.currentChatId = "";
      state.currentChat = null;
      localStorage.removeItem(ACTIVE_CHAT_STORAGE);
      await ensureCurrentChat();
      return;
    }

    dom.connectionDot.className = "connection-dot online";
    dom.connectionText.textContent = `${state.settings.model || "Model"} · connected${websocketIsReady() ? "" : " · polling"}`;
    renderAll();
  } catch (error) {
    if (state.connected) console.warn("Felix background sync failed", error);
  } finally {
...
function renderConversation() {
  ...
  const hasContent = messages.length > 0 || queue.length > 0;
  dom.emptyState.hidden = hasContent;
  dom.messageList.hidden = !hasContent;
  renderGreeting();
  if (!hasContent) { dom.messageList.replaceChildren(); return; }
...
function createQueuedMessageElement(item) {
  const article = document.createElement("article");
  article.className = "message-row user queued-message";
  article.dataset.queueItemId = item.id || "";
  const body = document.createElement("div");
  body.className = "message-body";
  const meta = document.createElement("div");
  meta.className = "queued-message-meta";
  meta.textContent = "Queued";
  const content = document.createElement("div");
  content.className = "message-content";
  content.textContent = item.prompt || (item.attachment_ids?.length ? "Analyze attached files" : "");
...
function renderComposer() {
  ...
  dom.composerStatus.textContent = busy
    ? `${busyHere ? (state.activeRun.stage || "Working") : "Another chat is working"} · new messages queue`
...
async function submitPrompt(event) {
  ...
  if (!state.currentChat || state.submitInFlight) return;
  ...
  renderComposer();
  try {
    const result = await api(`/api/chats/${encodeURIComponent(state.currentChat.id)}/submit`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
    if (result?.item && !state.currentChat.queue?.some(item => item.id === result.item.id)) {
      state.currentChat.queue = [...(state.currentChat.queue || []), result.item];
    }
    renderAll();
    await loadChat(state.currentChat.id, false);
    scheduleServerSync(120);
  } catch (error) {
...
function connectWebSocket() {
  ...
  socket.onopen = () => {
    socket.send(JSON.stringify({ token: state.apiKey }));
    state.websocketPingTimer = setInterval(() => {
...
async function handleSocketEvent(event) {
  ...
  if (event.type === "auth_ok") {
    ...
    state.activeRun = event.active || state.activeRun;
    dom.connectionText.textContent = `${state.settings.model || "Model"} · connected`;
    renderAll();
    scheduleServerSync(1200);
    return;
  }
  ...
  if (["chat_created", "chat_restored"].includes(event.type)) {
    scheduleChatRefresh();
    return;
  }
